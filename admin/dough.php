...
if(isset($_POST['add'])){ 
   ...
   $frosting = mysqli_real_escape_string($conn, $_POST['frosting']);
	...
	$file = mysqli_real_escape_string($conn, $_POST['file']);
	 ...
	 $insertsql = "INSERT INTO `yummy`.`dough` ( `title`, `duration`, `ingredient`, `frosting`, `direction`, `nutrition`,  `serving`) VALUES ('$title', '$duration', '$ingredient', '$frosting', '$direction', '$nutrition', '$serving')";
		
		 $run_query = mysqli_query($conn,$insertsql);
		if($run_query){
			header("Location:dough.php");
		}else{
...
if(isset($_GET['featured'])){
    ...
    $featuredsql = "UPDATE dough SET featured = '$featured' WHERE id = '$id'";
...
<div class="col-sm-8">
	<h1 class="yellow">Yeasted Dough Products </h1> 
...
<table class="container" id="dataTables-example">
    ...
    <tbody>
        ...
        <tr>
        <td>
        <a href="editdough?edit=<?php echo $product['id']; ?>" class="btn btn-xs btn-default"><span class="glyphicon glyphicon-pencil"></span></a>
       <a class="delete_dough btn btn-xs btn-default" data-emp-id="<?php echo $product['id']; ?>" href="javascript:void(0)">
<span class="glyphicon glyphicon-remove"></span>
        ...
        <td>
            <a href="dough?featured=<?php echo (($product['featured'] == 0)?'1':'0'); ?>&id=<?=$product['id']; ?>" class="btn btn-xs btn-default"><span class="glyphicon glyphicon-<?php echo (($product['featured']==1)?'minus':'plus'); ?>"> </span></a>&nbsp; <?php echo (($product['featured'] == 1)?'Featured Product': '');?>    
        </td> 
 ...
 <div class="row">
  ...
  <form action="dough" method="post" enctype="multipart/form-data" id="pro-form" style="display:none;" autocomplete="off" class="prod-form">
     <h1>Add Dough product</h1>
      <div class="form-group col-sm-4">
        ...
        <div class="form-group col-sm-4">
            ...
            <div class="form-group col-sm-3">
            <a href="dough.php" class="btn btn-danger" role="button">Cancel</a>
