...
if(isset($_POST['add'])){ 
   ...
   $frosting = mysqli_real_escape_string($conn, $_POST['frosting']);
	...
	$file = mysqli_real_escape_string($conn, $_POST['file']);
	 ...
	 $insertsql = "INSERT INTO `yummy`.`cupcake` ( `title`, `duration`, `ingredient`, `frosting`, `direction`, `nutrition`, `serving`) VALUES ('$title', '$duration', '$ingredient', '$frosting', '$direction', '$nutrition', '$serving')";
...
$sql = "SELECT * FROM cupcake WHERE deleted = 0 ";
$presult = $db->query($sql);
if(isset($_GET['featured'])){
    $id = (int)$_GET['id'];
    $featured = (int)$_GET['featured'];
    $featuredsql = "UPDATE cupcake SET featured = '$featured' WHERE id = '$id'";
...
<div class="col-sm-8">
	<h1 class="yellow">CupCake Products</h1> 
...
<table class="container" id="dataTables-example">
    ...
    <tbody>
        ...
        <tr>
        <td>
        <a href="editcupcake.php?edit=<?php echo $product['id']; ?>" class="btn btn-xs btn-default"><span class="glyphicon glyphicon-pencil"></span></a>
        <a class="delete_cupcake btn btn-xs btn-default" data-emp-id="<?php echo $product['id']; ?>" href="javascript:void(0)">
 ...
 <div class="row">
 	<div class="col-sm-2"></div>
 	<div class="col-sm-8 ">
  <form action="cupcake" method="post" enctype="multipart/form-data" id="pro-form" autocomplete="off" style="display:none;" class="prod-form">
